...
class AttachmentController extends BaseController
{
    ...
    /**
     * 获取阿里云 OSS 临时签名访问链接（1 小时有效）。
     *
     * 根据文件 ID 或 sign 查找文件，使用阿里云 OSS SDK signUrl 生成
     * 带 OSSAccessKeyId / Expires / Signature 的签名访问链接。
     * 仅当 OSS 类型为阿里云且已启用时可用。
     *
     * 请求参数：
     * - file_id：文件 ID（与 sign 二选一）
     * - sign：文件签名（与 file_id 二选一）
     */
    public function getStsAccessUrl(Request $request, Response $response): Response
    {
        // 获取登录用户
        $loginUser = [];
        if ($error = $this->requireLoginUser($request, $response, $loginUser)) {
        ...
        // file_id 和 sign 至少提供一个
        if ($fileId <= 0 && empty($sign)) {
            return $this->error($response, 10101, '请提供 file_id 或 sign 参数');
        }
        ...
        if (!$file) {
            return $this->error($response, 10101, '文件不存在');
        }
        ...
        // 从文件 URL 提取 OSS 对象路径
        $objectKey = OssHelper::extractObjectKeyFromUrl($realUrl);
        if (empty($objectKey)) {
            return $this->error($response, 10101, '无法解析文件的对象路径');
        }

        // 生成阿里云 OSS 签名 URL（1 小时有效）
        $signedUrl = OssHelper::generateAliyunSignedUrl($objectKey);
        if (!$signedUrl) {
            return $this->error($response, 10101, '生成临时访问链接失败，请确认 OSS 类型为阿里云');
        }

        return $this->success($response, [
            'url'          => $signedUrl,
            'expires_in'   => 3600,
            'file_id'      => (int) ($fileData['file_id'] ?? 0),
